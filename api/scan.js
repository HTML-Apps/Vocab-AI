...
// Empfängt ein Base64-Bild, ruft OpenAI gpt-4o-mini auf und gibt
// ein JSON-Array mit Vokabelpaaren zurück.
...
export default async function handler(req, res) {
  ...
  // ── Request-Body lesen ────────────────────────────────────────────
  const { image } = req.body; // Erwartet: { image: "data:image/jpeg;base64,..." }

  if (!image) {
    return res.status(400).json({ error: 'Kein Bild übermittelt.' });
  }

  // Base64-String bereinigen (Data-URL-Prefix entfernen, falls vorhanden)
  ...
  try {
            ...
            content: [
              {
                ...
                image_url: {
                  ...
                  detail: 'high', // 'high' für bessere OCR-Qualität
                },
              },
              {
                type: 'text',
                text: 'Extrahiere alle Vokabelpaare aus diesem Bild als JSON-Array.',
              },
            ],
          },
        ],
    ...
    // ── JSON parsen (Markdown-Blöcke abfangen, falls das Modell welche schickt) ──
    ...
    // ── Erfolg: Array ans Frontend zurückgeben ────────────────────────
    return res.status(200).json({ pairs: vocabPairs });
  } catch (err) {
